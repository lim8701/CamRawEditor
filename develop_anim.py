...
def marks(snap):
    """타임라인 눈금 — **활성/스킵 전부**. 스킵된 단계는 `active=False` 이고 t0/t1 이 -1 이다.

    ⚠️읽는 쪽이 `active` 로 걸러야 한다. 줄 위에 자리가 없어 -1 을 주는 것이므로, 걸러내지
      않으면 눈금이 트랙 왼쪽 밖에 쌓인다(중립 스냅샷에서 9개). "흐리게 표시" 로 적어 뒀던
      시절이 있었지만 그건 구현되지 않았다 — 지금은 **감춘다**.
    """
