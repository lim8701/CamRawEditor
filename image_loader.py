...
# ⚠️알려진 한계 — 선형 sRGB 원색은 von-Kries(채널별 게인)에 좋은 공간이 아니다. 실측 rel_gain
#   blue 를 실제 카메라 매트릭스와 비교하면 5500K 1.0배 / 4000K 1.2배 / 3000K 1.6배 / 2500K 2.4배 /
#   **2000K 11.2배**(28.7 vs 2.57) 로 저역에서 무너진다(고역은 반대로 순함 — 12000K 0.87배).
#   → QML tempSlider 가 일반 이미지에서 **하한 2500K** 로 범위를 줄여 대응한다(Main.qml 주석).
#   ⚠️Bradford LMS 로 바꾸면 2000K 가 5.50 까지 내려오지만, L·M 상관이 커서 8bit 프록시 왕복
#   오차가 실사진 2.0→8.3 code(랜덤 표본 2.0→39.1)로 뛴다 → **측정 후 기각**. 되살리려면
#   프록시를 16bit 로 올리는 게 전제(셰이더 텍스처 포맷 변경 — 별건).
