...
- **컨택트 시트**(빈 캔버스의 폴더 격자): **클릭=선택 / 더블클릭=열기**(탐색기와 같은 규칙),
  ...
  ★**썸네일은 위에서부터 채운다** — Qt 는 이미지 작업 큐를 **뒤에서부터** 처리해서 격자를 켜면
  아래 칸부터 떴다(실측: index 증가 2 / 감소 16). 칸이 **앞 칸의 완료를 기다리게** 해서 되돌렸다
  (`loadedCount`/`loadWindow`, 수정 후 증가 16 / 감소 2). ⚠️고정 지연(칸당 12ms)으로는 부족했다 —
  한 장 디코딩이 더 길어 여러 칸이 동시에 큐에 쌓이고 그 안에서 다시 뒤집힌다.
  ⚠️게이트를 **절대 index 로 걸면 교착**이다(보이는 칸만 만들어지므로 앞 칸이 없다) — 화면의
  첫 칸 기준으로 잰다. 좌측 탐색기 목록도 같은 게이트를 쓴다(둘 다 실측 감소 0회).
  ...
  요청은 전부 **`win.thumbPx()` 를 거쳐 DPR 로 나눈다**(요청 3ms 로 떨어짐). EXIF 썸네일 원본이
  기종 무관 **160x120** 이라 그 이상 요청해도 확대뿐이므로 잃는 것이 없다.
