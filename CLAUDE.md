...
| `exif_pass.py` | **원본 EXIF 통과**(export JPEG 한정) — 소스 APP1 을 파싱해 남길 것만 골라 **재직렬화**한다. 쓰기는 `pipeline._ifd_block` 재사용, 이 모듈이 더하는 건 **파서**다. ★⚠️`pipeline` 을 모듈 레벨에서 import 하므로 **`pipeline` 쪽 import 는 반드시 함수 안**(순환 import). ★⚠️**바이트순서를 안 보고 값을 뒤집으면 안 된다** — `II` 소스에서 `0x8769`(ExifIFD 포인터)가 깨져 **서브 IFD 전체(MakerNote 63개 포함)가 조용히 0개**가 됐다(에러 없음). ★⚠️MakerNote 는 **화이트리스트**(`_MAKERNOTE_SAFE`, 현재 후지만) — 후지는 self-relative 라 이동해도 63/63 그대로지만 **캐논은 TIFF 헤더 기준 오프셋이라 깨진다**(EOS R6 실측 114개 중 39개만 일치). ⚠️**태그 개수는 그대로라 개수 검증으로는 못 잡는다 — 값을 대조할 것.** 확인 안 된 제조사는 싣지 않는다(조용히 틀린 메타데이터 > 없는 것). ⚠️실패하면 None 을 돌려 `_exif_app1` 크레딧 전용으로 폴백 — 메타데이터 때문에 산출물을 깨뜨리지 않는다. 경위·측정·태그 정책은 `docs/exif_passthrough.md` |
...
| `sam_seg.py` | ⚠️**휴면 — 호출자가 없다.** 클릭 선택 마스킹(SlimSAM-77 ONNX). 배선은 `afef809` 에서 철회하고(Masking 패널 v1.5.0 복원) **파일만 남겼다** — 실수로 남은 잔해가 아니다. spec 에도 없어 배포본에 안 들어간다. 되살릴 때 그대로 쓰도록 계약이 살아 있다 — 다운로드 3종(`is_ready`/`ensure_model`/`provider_label`)은 `caption.py` 와 동형, `encode`(이미지당 1회)/`decode`(클릭마다)는 SAM 고유다. ⚠️`sky_seg.py` 와 같다고 읽지 말 것 — 그쪽은 `ensure_model` 만 같고 `is_ready` 대신 `model_available` 이다. `face_seg.py` 의 "sam_seg 패턴"(다운로드 진행률 = 2파일 합산 바이트)이 가리키는 것이 이 파일이다 |
...
- **컨택트 시트**(빈 캔버스의 폴더 격자): **클릭=선택 / 더블클릭=열기**(탐색기와 같은 규칙),
  ...
  ⚠️**썸네일은 아래 칸부터 뜬다**(Qt 가 이미지 작업 큐를 뒤에서부터 처리한다). 순서를 잡으려고
  "앞 칸의 완료를 기다리는 게이트" 를 넣었다가 **연달아 세 번 사용자 보고**를 내고 되돌렸다 —
  뷰 내부 상태(첫 칸이 무엇인지, delegate 생성·파괴 시점)에 얹혀야 하고 **헤드리스로 검증이
  안 된다**(오프스크린 뷰는 delegate 를 만들지 않는다). 아래 HiDPI 캡 덕에 한 장이 0.6ms 라
  한 화면이 한꺼번에 차고, 순서는 눈에 띄지 않는다. **다시 시도한다면 뷰를 게이팅하지 말고
  백그라운드 워커로 provider 캐시를 미리 데울 것.** 경위 전체는 `docs/ui_notes.md`.
  ★**썸네일 요청 크기 160 은 임의 값이 아니다** — 그 이하는 EXIF 썸네일(**0.6ms/장**), 넘으면
  임베드 프리뷰 축소 디코딩(**43.5ms/장, 72배**). 셀을 키우려면 이 비용을 먼저 정할 것.
  ★⚠️**`sourceSize` 는 논리 픽셀이라 Qt 가 DPR 을 곱해 provider 에 넘긴다** — 배율 125% 에서
  160 이 200 으로 도착해 위 빠른 경로를 통째로 벗어나 있었다(실측 요청 간격 73ms). 썸네일
  요청은 **`win.thumbPx()`**(= `min(px, floor(160/dpr))`)를 거친다 — ⚠️`round` 로 나누면
  150% 에서 161 이 되어 **캡이 아무 일도 안 한다**(160=0.6ms vs 161=43.5ms). ⚠️캡은 손실이
  없는 것이 아니라 **속도↔선명도 교환**이다 — 160 초과에서는 provider 가 임베드 프리뷰를
  디코딩해 실제로 더 선명하다(edge 320 → 213x320). 그래서 수백 장을 그리는 곳(시트·목록·태그)만
  캡을 걸고 **호버 피크는 예외**다. 상세는 `docs/ui_notes.md`.
