...
float w  = max(0.0, mix(1.0, sqrt(4.0*lg*(1.0-lg)), grainToneK));   // 미드톤 1.0
...
반전된다). K=1.28 에서는 표시 휘도 1.2% 이하 · 98.8% 이상에서만 걸린다.

### 실측 캘리브레이션 — 이 모델에서 유일하게 측정에 근거한 계수
...
**비대칭 지수 γ 는 도입하지 않았다.** 앙상블 피팅에서는 γ=0.84 가 잔차를 2.5배 더 줄였지만,
롤별로 보면 γ 가 **0.64~0.98 로 흔들린다** — 필름의 성질이 아니라 롤별 노출/스캔 차이를 흡수하고
있었다는 뜻이라 파라미터를 늘리지 않았다.

⚠️ 남는 불일치는 **섀도 끝**이다(휘도 32 에서 롤별 0.54~0.78). 특히 20241006 롤은 피크가
휘도 58 에 있어 혼자 다르다(노출 부족 프레임이 많거나 다른 스톡). 미드톤~하이라이트는 네 롤이
모두 잘 맞는다.
...
정확히 맞추는 0.184 대신 **`GRAIN=0.17`** 로 잡았고, 이제 슬라이더 최대에서 **9.22/255** 가 나온다.
...
| 파라미터 | 보존되는 것 | 정규화 |
|---|---|---|
| `GRAIN_TONE` | 미드톤 진폭 | `mix()` 기준점이 1.0 — 정규화 불필요 (K>1 이라 `max(0,·)` 필요) |
