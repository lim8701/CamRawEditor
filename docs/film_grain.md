...
```glsl
vec3 e    = /* 위의 3층 fBm — hash23 이 한 좌표에서 무상관 난수 3개를 낸다 */;
float k   = grainColor;                              // Color 슬라이더
float mono = (e.r + e.g + e.b) * inversesqrt(3.0);   // 층 공통 성분 = 휘도 요동
vec3 n = mix(vec3(mono), e, k) * nrm;
```

- `k=0` → 세 층 완전 상관 = **흑백 단층 필름**
- `k=1` → 세 층 완전 독립 = **컬러 필름의 물리**

`hash23`(Dave Hoskins)은 스칼라 해시를 세 번 부르는 것보다 싸다.
...
| `Color` (k) | **휘도** σ | `1/√((1−k)² + k²·\|LUMA\|² + 2(1−k)k/√3)` |

Color 의 식에 교차항이 붙는 이유는 `mono` 가 층들의 합이라 층 자신과 상관이 있기 때문이다.
셰이더는 슬라이더가 실시간으로 변하므로 이 식을 인라인 계산하고(상수는 `dot(LUMA,LUMA)` ·
`inversesqrt(3.0)` 로 유도해 리터럴 중복 없음), numpy 쪽은 `coeffs.grain_color_norm(k)` 를 쓴다.
**수정 시 양쪽 동시.**
...
### 기본값을 Color 1.0 으로 두지 않은 이유

`k=1` 이 물리적으로는 맞지만 **색 얼룩이 휘도 그레인의 2.11 배**가 되어 필름이 아니라 고감도
디지털 노이즈처럼 보인다. 이 모델이 세 층에 같은 σ 와 같은 셀 크기를 주는데, 실제 필름은 층별
입상도가 다르고 염료 구름이 확산돼 색 얼룩이 훨씬 완화되기 때문이다. 그 전부를 모델링하는 대신
`k` 를 절충 다이얼로 두었다. **0.3 은 물리 유도값이 아니라 눈으로 고른 값이다.**

---

## 프리뷰 = Export

Export(`pipeline._grain_field`)는 셰이더의 `hash23`/`valueNoise3` 를 numpy 로 그대로 이식해
...
둘 다 연산 순서를 바꾸지 않아 순진한 평가와 **비트 단위로 동일**함을 검증했다. 결과적으로 3층
모델이 흑백 단층 모델(5.5s)보다 빠르다.
...
| Roughness / Color | 휘도 σ | 채도 σ (R−G) |
|---|---|---|
| 0.0 / 0.0 | 6.185 | 0.023 |
| 0.5 / 0.3 (기본) | 6.186 | 3.007 |
| 1.0 / 0.0 | 6.183 | 0.024 |
| 0.5 / 1.0 | 6.188 | 13.060 |
| 1.0 / 1.0 | 6.180 | 13.065 |

휘도 σ 가 모든 조합에서 6.18 로 고정 — 어떤 결을 고르든 세기는 변하지 않는다.

**그 외** — `hash23` 세 층 상관계수 −0.0017 / +0.0038 / −0.0010(무상관) · 스트립 결합 == 통째
렌더 · 구버전 사이드카(키 없음) 기본값 폴백 · 격자 최적화 == 순진한 평가(비트 단위).
...
2. **톤커브 뒤 display 공간에서 가산한다.** 실제 그레인은 톤커브 *앞* 의 밀도/노광 요동이다.
   ...
   가장 유력한 정공법 후보.
3. **층별 입상도/셀 크기가 동일하다.** 실제는 층마다 감도와 결정 크기가 다르고 염료 구름이
   확산된다. Color 슬라이더가 `k=1` 의 색얼룩 과잉을 누르는 인위적 다이얼인 이유가 이것이다.
