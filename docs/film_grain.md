...
| **모델** | 0.69 | 0.54 | 0.40 | 0.26 | **0.13** | **0.00** | **0.00** |
...
밝은 끝을 포함해 재피팅해도 **K=1.29, γ=0.90 으로 사실상 변화가 없다**(잔차 0.0442→0.0429).
계수가 아니라 **벨이라는 형태의 한계**다 — 실제 곡선은 피크 직후 더 빨리 떨어지고 끝에서 꼬리를
남기는데 대칭 벨은 그 둘을 동시에 못 낸다.
...
|---|---|---|
| 현재 (K, γ) | 0.00183 | — |
| + 바닥값 | 0.00183 | 측정 구간에서 미발동 = 무효 |
| 벨 지수 | 0.00128 | 1.98 ±0.46 |
| 양측 γ(피크 위/아래 별도) | **0.00076** | 0.61 ±0.14 (0.39~0.75) |

양측 γ 가 가장 잘 맞지만 ① 3번째 파라미터가 롤마다 **±23%** 로 기존 γ(±13%)보다 불안정하고
② 개선폭이 **0.8/255**(휘도 248 에서 우리 1.4 vs 실측 2.2, 전 범위의 0.3%)로 지각 한계 아래다.
셰이더·pipeline 양쪽에 파라미터를 하나 더 얹고 동기화할 값어치가 없다고 판단했다.

**결론: 휘도 250 부근에서 우리 그레인은 실제 필름보다 약 0.8/255 만큼 깨끗하다.** 자료에서
"하늘 σ 0.2" 같은 수치를 인용할 때 이 점을 감안할 것 — 실제 필름의 하늘은 완전히 깨끗하지 않다.
...
**두 방향이 동시에 일어난다** — 하늘의 얼룩은 사라지고(σ 3.3 → 0.2) 나무의 입자감은 60% 진해진다
(5.5 → 8.8). 진폭이 톤의 함수이기 때문이다. 균일 진폭으로는 이 둘을 동시에 얻을 수 없다:
