...
# 필름 그레인 강도(슬라이더 1.0 에서의 천장). 실측 기준: Noritsu 스캔 4롤의 미드톤 σ 가
# 9.65~10.41/255 인데 0.12 일 때 우리 최대치는 6.51 로 약 65% 였다. 스캐너 샤프닝이 스캔 σ 를
# 부풀렸을 것(1.2~1.5× 추정)이라 정확히 맞추는 0.184 는 과하다고 보고 그 사이로 잡음.
# 정당화 가능한 범위 0.14~0.18 — 취향에 따라 이 안에서 조정.
GRAIN = 0.17
...
# ★실측 기반 유일한 그레인 계수: Noritsu 필름 스캔 4롤·151프레임의 평탄 패치 11,512개에서
#   σ vs 휘도를 재고 w=mix(1, √(4l(1−l)), K) 를 피팅 → 롤별 K = 1.19/1.23/1.32/1.37 (평균 1.28,
#   편차 5.5%). 이전 눈대중 값 0.70 대비 잔차 5~16× 개선. 비대칭 지수 γ 도 시도했으나 롤마다
#   0.64~0.98 로 흔들려(필름 성질이 아니라 롤별 노출/스캔 차이) 도입하지 않음. docs/film_grain.md
# ⚠️K>1 이면 끝단에서 w<0 → 셰이더·pipeline 양쪽에서 max(0,·) 클램프 필수(휘도 1.2%↓·98.8%↑).
GRAIN_TONE = 1.28
